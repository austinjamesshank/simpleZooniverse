...
def printSubjectInfo(imgs, metadata, dest, lock):
    ...
    lock.acquire()
    try:
        print('{}\t{}'.format(str(imgs), str(metadata)), file=open(dest + '/' + 'subject.aus', 'a'))
    finally:
        lock.release() 
        
# --- --- --- --- --- --- --- ---
     
def run(username, password, projectName, dsLocations, customArgs={}, verbose=False, projID=-1, tutorial=False):
